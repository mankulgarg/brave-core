...
class InsertTokensJob : public BATLedgerJob<bool> {
 public:
  void Start(const std::vector<ContributionToken>& tokens,
    ...
    // TODO(zenparsing): Many of these fields are required at the database level
    // but aren't important for this use case, so we're just entering dummy
    // data.
...
Hold::Hold(base::WeakPtr<BATLedgerContext> context,
  ...
  // TODO(zenparsing): When we hold tokens, should we notify the observer that
  // the available balance has been updated?
  auto& manager = context->Get<ContributionTokenManager>();
  for (auto& token : tokens_) {
    manager.reserved_token_ids_.insert(token.id);
  }
}
...
void Hold::Release() {
  if (parent_) {
    ...
    auto& manager = context_->Get<ContributionTokenManager>();
    for (auto& token : tokens_) {
      manager.reserved_token_ids_.erase(token.id);
    }
  }
