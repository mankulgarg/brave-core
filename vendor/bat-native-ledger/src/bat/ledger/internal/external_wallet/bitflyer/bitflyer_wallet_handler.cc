...
class AuthJob : public BATLedgerJob<absl::optional<ExternalWallet>> {
 public:
  void Start(const base::flat_map<std::string, std::string>& auth_params) {
    context().GetLedgerImpl()->bitflyer()->WalletAuthorization(
 ...
 private:
  void OnCompleted(mojom::Result result,
    ...
    auto wallet = context().GetLedgerImpl()->bitflyer()->GetWallet();
...
std::string BitflyerWalletHandler::GetAuthorizationURL() {
  return context().GetLedgerImpl()->bitflyer()->GetWallet()->login_url;
}

Future<absl::optional<ExternalWallet>>
BitflyerWalletHandler::HandleAuthorizationResponse(
