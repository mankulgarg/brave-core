...
v8::Local<v8::Promise> JSSolanaProvider::Disconnect(gin::Arguments* arguments) {
  ...
  ALLOW_UNUSED_LOCAL(resolver.ToLocalChecked()->Resolve(
...
void JSSolanaProvider::SendResponse(
  ...
  if (success) {
    ALLOW_UNUSED_LOCAL(resolver->Resolve(context, response));
  } else {
    ALLOW_UNUSED_LOCAL(resolver->Reject(context, response));
  }
}
